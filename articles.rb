...
source[:response][:results].each do |result|
  result[:views] = 0
end

def read_article(source)
  articles = (source[:response][:results].length)-1
  random_article = rand(0.. articles)
  source[:response][:results][random_article][:views]+=1
end

def display_views(source)
  source[:response][:results].each do |result|
    puts result[:webTitle]
    puts "has been viewed #{result[:views]} times."
  end
end
